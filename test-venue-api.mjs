...
const VENDOR_EMAIL = 'testvendor@example.com'; // Test vendor email
const VENDOR_PASSWORD = 'test123456'; // Test vendor password

// Optional: If you have an existing token, set it here to skip login
// const EXISTING_TOKEN = 'your-token-here';
...
async function loginVendor() {
  try {
    ...
    console.log('🔐 Using endpoint:', `${BASE_URL}/auth/login`);
...
async function runTests() {
  try {
    ...
    // Test 1: Login or use existing token
    let token = EXISTING_TOKEN;
    if (!token) {
      console.log('⚠️  No existing token provided. Attempting login...');
      console.log('⚠️  Note: Make sure vendor credentials are correct in test-venue-api.mjs\n');
      token = await loginVendor();
    } else {
    ...
    console.error('\n❌ Tests failed!');
    console.error('\n💡 Tips:');
    console.error('   1. Make sure backend server is running on port 8030');
    console.error('   2. Update VENDOR_EMAIL and VENDOR_PASSWORD in test-venue-api.mjs');
    console.error('   3. Or set EXISTING_TOKEN if you have a valid token');
    process.exit(1);
  }
}
